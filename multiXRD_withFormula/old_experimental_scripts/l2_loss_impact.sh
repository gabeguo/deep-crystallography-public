...
# Almost exactly the same as generate_base_model.sh, except that we use L2 loss (verify with diffcheck)

cd ../

for CRYSTAL_SYSTEM in Cubic Trigonal
do
    OUTPUT_FOLDER=/data/therealgabeguo/crystallography/paper_results/${CRYSTAL_SYSTEM}_l2Loss
    MODEL_PATH=$OUTPUT_FOLDER/${CRYSTAL_SYSTEM}_l2Loss_model.pt
    RESULTS_PATH=$OUTPUT_FOLDER/${CRYSTAL_SYSTEM}_l2Loss_visualizations
