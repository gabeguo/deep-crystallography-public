# Usage: CUDA_VISIBLE_DEVICES=x bash ignore_chem_ratios_standardized_impact.sh [Cubic (or) Trigonal]
# uses GAN, as that was shown to be slightly better than not using GAN

cd ../../

# have a number for presence of element, 0 for absence

for CRYSTAL_SYSTEM in $1 #Cubic Trigonal
do
    OUTPUT_FOLDER=/home/gabeguo/data/crystallography_paper_version/paper_results/${CRYSTAL_SYSTEM}_ignoreChemicalRatiosStandardized
    MODEL_PATH=$OUTPUT_FOLDER/${CRYSTAL_SYSTEM}_ignoreChemicalRatiosStandardized_model.pt
    RESULTS_PATH=$OUTPUT_FOLDER/${CRYSTAL_SYSTEM}_ignoreChemicalRatiosStandardized_visualizations
